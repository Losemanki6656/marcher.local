<template>
    <a-card class="page-content-sub-header" :bodyStyle="{ padding: '0px', margin: '0px 16px 0' }">
        <a-row>
            <a-col :span="24">
    ...
    <a-card class="page-content-container">
        <a-typography-text type="warning" strong :style="{ fontSize: '18px' }">
            {{ $t("online_orders.online_store_url") }}
        </a-typography-text>
        <a-row :gutter="[8, 8]" class="mt-5">
            <a-col :xs="24" :sm="24" :md="12" :lg="6" :xl="6">
                <a-input-group compact>
                    <a-input v-model:value="storeUrl" style="width: calc(100% - 100px)" />
                    <router-link :to="{
                        name: 'front.homepage',
        ...
        <a-row :gutter="[8, 8]" class="mt-20 mb-20">
            <a-col :xs="24" :sm="24" :md="12" :lg="6" :xl="4">
                <a-input-search style="width: 100%" v-model:value="filters.searchString" show-search :placeholder="$t('common.placeholder_search_text', [$t('stock.invoice_number')])
                    " />
            </a-col>
            <a-col :xs="24" :sm="24" :md="8" :lg="6" :xl="4">
                <a-select v-model:value="filters.user_id" :placeholder="$t('common.select_default_text', [
                    $t(`${orderPageObject.langKey}.user`),
                ])
                    " :allowClear="true" style="width: 100%" optionFilterProp="title" show-search>
                    <a-select-option v-for="user in users" :key="user.xid" :title="user.name" :value="user.xid">
                        {{ user.name }}
                    </a-select-option>
                </a-select>
            </a-col>
        ...
        <a-row>
            <a-col :span="24">
                <a-tabs v-model:activeKey="filters.order_status">
                    <a-tab-pane key="all" :tab="`${$t('common.all')} ${$t('menu.online_orders')}`" />
