<template>
    <div>
        ...
        <div id="printThis" style="display: none">
            <vue-barcode :value="itemCode + ''" :options="{ height: 75, format: barcodeSymbology }" tag="svg"
                v-for="n in 24" :key="n" />
        </div>

        <a-modal v-model:visible="visible" :title="$t('product.barcode')" :footer="false" @ok="handleOk">
            <p style="text-align: center">
                <vue-barcode :value="itemCode + ''" :options="{ height: 75, format: barcodeSymbology }"
                    tag="svg"></vue-barcode>
            </p>
